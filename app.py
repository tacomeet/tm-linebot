...
def handle_text_message(event):
    ...
    if text == 'quota':
        ...
        line_bot_api.reply_message(
            event.reply_token, [
                TextSendMessage(text=f'type: {quota.type}'),
                TextSendMessage(text=f'value: {str(quota.value)}'),
            ]
        )
        ...
        line_bot_api.reply_message(
            event.reply_token, [
                TextSendMessage(text=f'total usage: {quota_consumption.total_usage}'),
            ]
        )
    elif text == 'push':
        ...
        line_bot_api.multicast(
            [event.source.user_id], [
                TextSendMessage(text='MULTICAST'),
            ]
        )
    elif text == 'broadcast':
        line_bot_api.broadcast(
            [
                TextSendMessage(text='BROADCAST 1'),
                TextSendMessage(text='BROADCAST 2'),
            ]
        )
    elif text == 'confirm':
