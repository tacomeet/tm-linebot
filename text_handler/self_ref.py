...
def _self_ref(line_bot_api, user: User, event):
    ...
    if text == ms.self_ref.M_1_EXP:
        ...
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=ms.self_ref.M_EXP_1))
        line_bot_api.push_message(user.get_id(), TextSendMessage(text=ms.self_ref.M_EXP_1_EX))
