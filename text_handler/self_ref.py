...
def _self_ref(line_bot_api, user: User, event):
    ...
    if text == message.self_ref.M_1_EXP:
        user.set_session_type(StatusType.SELF_REF_EXP)
        user.set_session_stage(2)
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=message.self_ref.M_EXP_1))
        line_bot_api.push_message(user.get_id(), TextSendMessage(text=message.self_ref.M_EXP_1_EX))
    elif text == message.self_ref.M_1_PERS:
        user.set_session_type(StatusType.SELF_REF_PERS)
        user.set_session_stage(2)
        pass
    elif text == message.self_ref.M_1_VIS:
        user.set_session_type(StatusType.SELF_REF_VIS)
        user.set_session_stage(2)
        pass
    elif text == message.self_ref.M_1_TURN:
